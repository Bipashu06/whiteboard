...
const handleMouseMove = (event) => {
  ...
  if (draggingImageId !== null && imageResizing !== true) {
    ...
    console.log(imageLastX, droppedImages)
    setDroppedImages((prevImages) =>
        prevImages.map((image) =>
            image.id === draggingImageId
    ? {
        ...
        height: newHeight, // Dynamically update height
    }
    : image
)
)
...
//------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    const saveCanvasAsImage = () => {
        // const canvas = canvasRef.current;
        // if (canvas) {
        //     const image = canvas.toDataURL('image/png'); // Convert canvas to a PNG data URL
        //     const link = document.createElement('a');
        //     link.href = image;
        //     link.download = 'canvas-image.png'; // File name for the downloaded image
        //     link.click(); // Trigger the download
        // }
        const canvasContainer = document.getElementById('draw-area'); // Assuming you have a container wrapping canvas and divs

            // Use html2canvas to capture the canvas container (including divs)
        html2canvas(canvasContainer, { scale: 3, logging: true, useCORS: true }).then((canvas) => {
        // Convert captured content to JPEG
        const image = canvas.toDataURL('image/jpeg'); // Convert the canvas to a JPEG data URL
        const link = document.createElement('a');
        link.href = image;
        link.download = 'canvas-image.jpg'; // File name for the downloaded image
        link.click(); // Trigger the download
    });
    };
                ...
                <div className="top-nav">
                    <div className="page-name-container">
                        <div
                            ...
                            dangerouslySetInnerHTML={{ __html: content }} // Render content safely
                        />
                    ...
                    {droppedImages.map((image, index) => {
                        ...
                        return (
                            ...
                            {isSelected && (
                                ...
                                className="resize-controller"></div>
                            )}
